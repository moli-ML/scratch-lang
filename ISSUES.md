...
- [x] ~~新功能缺少测试~~ (已添加扩展导入、块注释、多行字符串等测试)
...
### 2.2 正则表达式可维护性差
- [x] ~~`blocks.py` 中大量复杂正则，缺少注释说明~~ (已添加文件头注释说明)
...
### 4.1 积木支持不完整
- [x] ~~缺少自定义积木（自制积木）支持~~ (已添加，支持定义和调用)
- [x] ~~缺少扩展积木支持（音乐、视频侦测等）~~ (已添加音乐扩展积木)
- [x] ~~缺少云变量支持~~ (已添加 `云变量:` 语法)

### 4.2 资源处理
- [x] ~~没有验证图片/音频文件格式~~ (已添加文件头魔数验证)
- [x] ~~没有处理文件不存在的情况（只打印警告）~~ (已改为抛出异常)
- [x] ~~SVG 文件的 rotationCenter 计算可能不准确~~ (已修复，解析 SVG 尺寸计算中心点)

### 4.3 语法限制
- [x] ~~不支持多行字符串~~ (已添加 """...""" 三引号语法)
- [x] ~~不支持转义字符~~ (已添加 \n \t \r \\ \" \' 支持)
- [x] ~~注释只支持 `//`，不支持 `/* */`~~ (已添加块注释支持)

### 4.4 更多功能
- [x] ~~加入语法树可视化功能（在侧边栏中显示，双击显示用法）~~ (已添加 syntax_tree.py，显示所有可用积木)
- [x] ~~加入自定义扩展js支持（导入外部扩展JS文件）~~ (已添加扩展导入功能)
- [x] ~~加入#code#后直接编写js功能（#end#结束，使用自定义扩展实现）~~ (已添加 _extract_js_blocks)
- [x] ~~加入语法中英文切换功能~~ (已为主要积木添加英文语法支持)
...
- [ ] 复杂逻辑缺少行内注释
- [x] ~~`AGENTS.md` 内容不明确~~ (内容已完善)

### 5.2 用户文档
- [x] ~~缺少完整的语法参考手册~~ (已添加 docs/SYNTAX.md)
- [x] ~~缺少错误代码对照表~~ (已添加 docs/ERRORS.md)
